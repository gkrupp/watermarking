...
if args.m == 'RHFM':
    method = RadialHarmonicFourierMoment(max_order, W, qs=args.qs, Vfile='../../V_RHFM_256.h5')
elif args.m == 'ZM':
    method = ZernikeMoment(max_order, W, qs=args.qs, Vfile='../../V_ZM_256.h5')
elif args.m == 'PZM':
    method = PseudoZernikeMoment(max_order, W, qs=args.qs, Vfile='../../V_PZM_256.h5')
else:
    method = None

df = run(method, images, Ls, repetitions, gen_bs, '../../images/monochrome/', multiproc=False)

df.to_csv(str(method.name)+'_'+str(args.qs)+'.csv', index=False)
